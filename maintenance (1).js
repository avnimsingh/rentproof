...
const formalCheckbox = document.getElementById("formalWording");
const formalPreview = document.getElementById("formalPreview");
const titleInput = document.getElementById("title");
const descInput = document.getElementById("description");

function buildFormalText() {
  const today = new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" });
  const title = titleInput.value.trim() || "[issue]";
  const desc = descInput.value.trim() || "[details]";
  return `On ${today}, I am formally reporting the following maintenance issue: ${title}.\n\nDetails: ${desc}\n\nPlease let me know when this can be addressed. This message serves as written notice of this request, sent on the date above.`;
}

function updatePreview() {
  if (formalCheckbox.checked) {
    formalPreview.style.display = "block";
    formalPreview.textContent = buildFormalText();
  } else {
    formalPreview.style.display = "none";
  }
}

formalCheckbox.addEventListener("change", updatePreview);
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    const finalDescription = formalCheckbox.checked ? buildFormalText() : descInput.value.trim();
